...
Pipeline:
  ...
  summary_net    → SequenceNetwork (LSTM) compresses fixation sequences
...
Install:
    ...
    # OR: pip install bayesflow torch
...
try:
    ...
    BF_OK = False
    print("BayesFlow not found. Install: pip install bayesflow tensorflow")
...
def train(word_lengths_list: list,
    ...
    """
    Train the BayesFlow amortized posterior estimator for SWIFT.

    Parameters
    ----------
    word_lengths_list     : list of np.ndarray (word lengths per sentence)
    word_freqs_list       : list of np.ndarray (word freqs per sentence)
    n_epochs              : training epochs
    batch_size            : simulations per gradient step
    num_batches_per_epoch : gradient steps per epoch
    seq_len               : padded fixation sequence length
    ...
    """
    if not BF_OK:
        raise ImportError("Install BayesFlow: pip install bayesflow tensorflow")

    # Register corpus
    set_corpus(word_lengths_list, word_freqs_list, seq_len)

    print("\n" + "=" * 55)
    print("SWIFT — BayesFlow v2 Training")
    print("=" * 55)
    ...
    print(f"  theta shape    : {test_batch['theta'].shape}")
    print(f"  fixations shape: {test_batch['fixations'].shape}")
    ...
    # Summary network: LSTM over fixation sequences
    # SequenceNetwork handles variable-length time series natively
    summary_network = bf.networks.SequenceNetwork(
        summary_dim  = 32,       # embedding dimension (must be >= n_params)
        bidirectional = False,   # unidirectional: reading is left-to-right
    )

    # Inference network: normalizing flow to approximate P(theta | embedding)
    # CouplingFlow with spline transforms is flexible and fast
    inference_network = bf.networks.CouplingFlow(
        transform  = "spline",  # more expressive than affine
        num_layers = 6,         # flow depth
    )
    ...
    workflow = bf.BasicWorkflow(
        ...
        standardize       = ["inference_variables"],   # standardise parameters
    )
...
def run_inference(workflow,
    ...
    ----------
    workflow         : trained bf.BasicWorkflow
    fixation_array   : shape (n_fixations, 3)
                       columns: (word_id_norm, landing_norm, duration_s)
    n_posterior_samples : posterior samples to draw

    Returns
    -------
    samples : np.ndarray shape (n_posterior_samples, 4)
              in original parameter scale
    """
    seq_len = _SEQ_LEN
    ...
    # BayesFlow v2 sample() call
    # conditions dict must use same keys as simulator output
    post_draws = workflow.sample(
        conditions  = {"fixations": padded},
        num_samples = n_posterior_samples,
    )
    # post_draws["theta"] has shape (1, n_posterior_samples, 4)
...
def load_workflow(model_path: str) -> "keras.Model":
    """
    Load a previously saved approximator.

    Usage:
        approx = load_workflow("inference/trained_model/swift_approximator.keras")
        post   = approx.sample(conditions=..., num_samples=1000)
    """
    import keras
...
if __name__ == "__main__":
    ...
    if BF_OK:
        ...
        print("Install bayesflow first: pip install bayesflow tensorflow")
...
def train_offline(thetas:    np.ndarray,
    ...
    Parameters
    ----------
    thetas     : np.ndarray shape (n_sim, 4)       — normalised parameters
    seqs       : np.ndarray shape (n_sim, seq_len, 3) — fixation sequences
    wll        : word lengths list (needed for diagnostics later)
    wfl        : word freqs list   (needed for diagnostics later)
    n_epochs   : training epochs
    ...
    # ------------------------------------------------------------------
    # Build a dummy simulator that serves pre-generated data
    # BayesFlow v2 fit_dataset() accepts a keras dataset directly
    # ------------------------------------------------------------------
    import tensorflow as tf

    # Build tf.data.Dataset from pre-generated arrays
    dataset = (
        tf.data.Dataset
        .from_tensor_slices({
        ...
        .shuffle(buffer_size=len(thetas), seed=42)
        .batch(batch_size)
        .prefetch(tf.data.AUTOTUNE)
    )

    # ------------------------------------------------------------------
    # Adapter, networks, workflow — same as online training
    # ------------------------------------------------------------------
    adapter = build_adapter()
    ...
    # For offline training we still need a simulator object for the
    # workflow constructor — use a minimal pass-through
    simulator = bf.simulators.make_simulator([swift_prior, swift_likelihood])
...
# ---------------------------------------------------------------------------
# Rebuild workflow from saved model (for inference mode)
# ---------------------------------------------------------------------------

def rebuild_workflow(model_path: str) -> "bf.BasicWorkflow":
    """
    Rebuild a BasicWorkflow from a saved approximator for inference.

    Parameters
    ----------
    model_path : path to saved .keras file

    Returns
    -------
    workflow : bf.BasicWorkflow ready for workflow.sample()
    """
    if not BF_OK:
        raise ImportError("Install BayesFlow: pip install bayesflow tensorflow")
    ...
    # Rebuild workflow shell with same architecture
    simulator = bf.simulators.make_simulator([swift_prior, swift_likelihood])
    adapter   = build_adapter()
    ...
    # Load saved weights into the approximator
